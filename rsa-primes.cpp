            ...
            int num_digits)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int i = 0; !Miller(x, iteration,y); i++) {		//Miller returns true if number is prime
            RanNum(x, y);
        }
...
static void RanNum(mpz_t& x, int y) {
    ...
    int z;
    z = (rand() % 9) + 1;		//random number 1 to 9 since this is the first digit
    mpz_add_ui(x, x, z); // x += z

    for (int i = 1; i < y; i++) {

        mpz_mul_si(x, x, 10); // x *= 10	
        z = rand() % 10;			//random number 0 to 9 
        mpz_add_ui(x, x, z); // x += z
...
//Miller-Rabin 
static bool Miller(const mpz_t x, int iteration, int digits)	//iteration used for accuracy
{
    ...
    if ((mpz_cmp_si(x, 2) == 0) // x == 2		//check to see if the random number is a divisable number
    ...
    while (mpz_tdiv_ui(s, 2) == 0) // s % 2 == 0	//find a odd number smaller than the random number
    {
    ...
    mpz_t x_sub1;
    mpz_init(x_sub1);
    for (int i = 0; i < iteration; i++) 	//the random testing number could be a liar, iteration clears this up
    {
        RanNum(a, secNum);
 ...
 //modular exponentiation that does (x^y)%p
static void modExp(const mpz_t base, const mpz_t exponent, const mpz_t mod, mpz_t &ret)
{
